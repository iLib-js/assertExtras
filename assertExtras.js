            ...
            for (p in expected) {
                if (p && expected.hasOwnProperty(p)) {
                    if (typeof(actual[p]) === 'undefined') {
                        ...
                        assert.contains(actual[p], expected[p], message);
                    } else {
                        if (actual[p] != expected[p]) {
                            fail(actual, expected, message + " Expected is looking for property " + expected + " and actual does not contain it.", "contains", assert.notDeepEqual);
                        }
                    }
                }
            }
